...
async function startChooserTimer(gameId) { // default to 3 minutes if not provided
  ...
  const expiresAt = new Date(Date.now() + 60000); // 2 minutes from now
...
async function startGuessTimer(gameId, maximumGuessingTime = 60000) { // default to 2 minutes if not provided
