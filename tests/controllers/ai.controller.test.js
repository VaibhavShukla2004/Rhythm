...
describe("ai.controller", () => {
  ...
  describe("generateHint", () => {
    it("should return 400 when lyrics or hint is missing", async () => {
      ...
      expect(res.json).toHaveBeenCalledWith({
        message: "lyrics and hint required",
      });
    ...
    it("should generate hint successfully", async () => {
      ...
      expect(aiService.generatePayload).toHaveBeenCalledWith(
  ...
  describe("modifyLyrics", () => {
    ...
    it("should modify lyrics successfully", async () => {
      ...
      aiService.getAIResponse.mockResolvedValue(
        "mocked modified lyrics response",
      );

      await modifyLyrics(req, res, next);

      expect(aiService.generateModifyLyricsPayload).toHaveBeenCalledWith(
        "hickory dickory dock",
        "make it about food",
      );
      expect(aiService.getAIResponse).toHaveBeenCalledWith(mockPayload);
      expect(res.status).toHaveBeenCalledWith(200);
      expect(res.json).toHaveBeenCalledWith({
        modifiedLyrics: "mocked modified lyrics response",
      });
    });
  });
});
