...
async function generateRoomCode() {
  ...
  while (true) {
    //keeps on trying until unique code
    let code = "";

    for (let i = 0; i < 6; i++) {
      code += chars.charAt(Math.floor(Math.random() * chars.length)); //math.floor() rounds DOWN a number
    }
...
async function createRoom(hostId, maxPlayers) {
  ...
  if (alreadyHostInAnotherRoom)
    throw new Error("You are already hosting a room");
...
async function getRoomDetails(roomCode) {
  //shld be available to players during active state of room
...
async function joinRoom(roomCode, userId) {
  ...
  const alreadyJoined = room.players.some(
    (player) => player.userId.toString() === userId.toString(), //.some() loops through it and returns true if any item matches the condition.room.players is the array,for players as room.players it checks. Analogous to java stream
...
async function leaveRoom(roomCode, userId) {
  ...
  room.players = room.players.filter(
    //removes player where player.userId matches userId of leaver
    (player) => player.userId.toString() !== userId.toString(),
  );
  await room.save();
...
async function preGameValidation(roomCode, userId) {
  //validate room,host details
  const room = await Room.findOne({ roomCode });

  if (!room) throw new Error("Room not found");

  if (room.hostId.toString() !== userId.toString())
    throw new Error("Only host can start game"); //only a person whos a host can start game

  if (room.gameStatus === "in-progress")
    throw new Error("Game already in progress"); //person cannot start game if a game in the room is already happening
